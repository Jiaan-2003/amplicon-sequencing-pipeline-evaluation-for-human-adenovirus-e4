...
#SBATCH --job-name=viroconstrictor_KX384945_metrics
...
RESULTS_DIR="$PROJECT_DIR/results/adeno_E4/viroconstrictor/ds1641_KX384945/results/combined/by_sample"
METRICS_DIR="$PROJECT_DIR/results/adeno_E4/metrics/viroconstrictor_KX384945"

# Creates/checks output directory
mkdir -p "$METRICS_DIR"

# Defines output file
OUTPUT_FILE="$METRICS_DIR/viroconstrictor_KX_consensus_metrics.tsv"
...
do
    ...
    echo -e "${BARCODE}\tViroConstrictor\tKX384945\t${LENGTH}\t${A_COUNT}\t${C_COUNT}\t${G_COUNT}\t${T_COUNT}\t${N_COUNT}\t${PERCENT_N}" >> "$OUTPUT_FILE"

done

echo "ViroConstrictor KX384945 consensus metrics obtained"